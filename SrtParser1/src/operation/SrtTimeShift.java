...
public class SrtTimeShift extends SrtOperator {
            ...
            switch (shiftType){
                case SHIFT_TYPE_PLUS:
                    if(plusBoth(node,shiftMsecond)){
                        ...
                        if(!minusBoth(node,shiftMsecond)){
                            onOperateSrtNodesListener.onOperationFail(new Exception("操作失败提示：时间轴后移失败，回滚失败，程序退出"));
                            exit(-1);
                        }
                        onOperateSrtNodesListener.onOperationFail(new Exception("操作失败提示：时间轴后移失败"));
                    }
                    break;
                case SHIFT_TYPE_MINUS:
                    if(minusBoth(node,shiftMsecond)){
                        ...
                        //此处应当进行回滚，返回未被修改过的源srtNodes
                        if(!plusBoth(node,shiftMsecond)){
                            onOperateSrtNodesListener.onOperationFail(new Exception("操作失败提示：时间轴前移失败，回滚失败，程序退出"));
                            exit(-1);
                        }
                        onOperateSrtNodesListener.onOperationFail(new Exception("操作失败提示：时间轴前移失败"));
                    }
                    break;
            }
        }
    ...
    public void doTimeShift(List<SrtNode> srtNodes,SrtNode startNode, OnOperateSrtNodesListener onOperateSrtNodesListener){
        ...
        if(isFoundFirst){
            ...
            onOperateSrtNodesListener.onOperationFail(new Exception("操作失败提示：移动失败，起始节点不在字幕序列中！"));
        }
